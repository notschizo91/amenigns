...
/// Self-union followed by a tiny morphological close (dilate+erode by weld
/// mm): point-tangent strokes come out genuinely welded, not just touching.
[<Import("unionWeld", "./clipper-helpers.js")>]
let unionWeld (rings: Ring array) (weld: float) : Ring array = jsNative
...
/// Shapes -> oriented ring soup (outers positive, holes negative) — the form
/// nonzero-fill clipper booleans expect.
let shapeRings (shapes: Shape list) : Ring array =
    shapes
...
let unionShapes (shapes: Shape list) : Shape list =
    toShapes (unionAll (shapeRings shapes))
